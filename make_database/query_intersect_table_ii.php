...
echo '<pre>';
echo "PubMedID_A \t".'Citation_A'."\t";
echo 'Phenotype_A name'."\t";
echo 'GeneSet_A count'."\t".'GeneSet_A list';
echo "\t";

echo "PubMedID_B \t".'Citation_B'."\t";
echo 'Phenotype_B name'."\t";
echo 'GeneSet_B count'."\t".'GeneSet_B list';
echo "\t";

echo 'Intersect A:B count'."\t".'Intersect A:B GeneSet'."\t";

echo 'hyperG dist '."\t".'Common A %'."\t".'Common B %';
echo "\t";

echo 'Difference A count'."\t". 'Difference GeneSet_A';
echo "\t";
echo 'Difference B count'."\t". 'Difference GeneSet_B';
echo '</pre>';
...
while ($array_eachPHE_combData = mysql_fetch_assoc($result))
{
	...
	while ($OneGene_attributes_from_combData = mysql_fetch_assoc($result02))
	{
		...
		//$OneGenePPI_Gen_NN_unique_list[] = $OneGene_attributes_from_combData['OneGenePPI_Gen_NN_unique']; 
		//$OneGenePPI_Phy_NN_unique_list[] = $OneGene_attributes_from_combData['OneGenePPI_Phy_NN_unique']; 
...
for ( $counter = 1; $counter <= count($phenotype_gene_lists); $counter += 1) {//scroll through each geneset
	...
	for ( $counter02 = 1; $counter02 <= count($phenotype_gene_lists); $counter02 += 1) {//scroll through each geneset again
		...
		if( ($count < $count02) and ($PubMed_ID_lists["$count"] != $PubMed_ID_lists["$count02"]) ){
			...
			$intersect = array_intersect($phenotype_gene_lists["$count02"], $phenotype_gene_lists["$count"]);
			
			$differenceA = array_diff($phenotype_gene_lists["$count"], $phenotype_gene_lists["$count02"]);
			$count_diffA = count($differenceA);
			sort($differenceA);
			$diffA_str = implode(', ', $differenceA);
			$diffA_str = mysql_real_escape_string($diffA_str);	

			
			$differenceB = array_diff($phenotype_gene_lists["$count02"], $phenotype_gene_lists["$count"]);
			$count_diffB = count($differenceB);
			sort($differenceB);
			$diffB_str = implode(', ', $differenceB);
			$diffB_str = mysql_real_escape_string($diffB_str);	
			...
			$intersect_other_key = array_intersect($phenotype_gene_lists["$count"], $phenotype_gene_lists["$count02"]);
			...
			$p = pvalue($N,$M,$n,$m);
				if($intersect["0"] == 'no intersection result'){
					$m = 0;
					$p = 1;				
				}
...
echo $PubMed_ID_lists["$count"]."\t".$citation_lists["$count"]."\t";
echo $PheChem_lists["$count"]."\t";
echo $n."\t".$gene_strA;
echo "\t";

echo $PubMed_ID_lists["$count02"]."\t".$citation_lists["$count02"]."\t";
echo $PheChem_lists["$count02"]."\t";
echo $M."\t".$gene_strB;
echo "\t";

echo $m."\t".$intersect_str;
echo "\t";

echo $pq."\t".round(($m/$n)*100)."\t".round(($m/$M)*100);
echo "\t";

echo $count_diffA."\t". $diffA_str;
echo "\t";
echo $count_diffB."\t". $diffB_str.'<br>';
echo '</pre>';
